...
class Exercise(AbstractSubmissionModel, AbstractLicenseModel, models.Model):
    ...
    name_original = models.CharField(
        max_length=200, verbose_name=_('Name'), default='')
    '''The exercise's name, as entered by the user'''

    muscles = models.ManyToManyField(
        Muscle, blank=True, verbose_name=_('Primary muscles'))
    ...
    equipment = models.ManyToManyField(
        Equipment, verbose_name=_('Equipment'), blank=True)
    ...
    uuid = models.CharField(
        verbose_name='UUID', max_length=36, editable=False, default=uuid.uuid4)
    '''
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_main:
            ...
            if ExerciseImage.objects.accepted().filter(
                exercise=self.exercise).count() == 0 or not \
                ExerciseImage.objects.accepted().filter(
                    exercise=self.exercise, is_main=True).count():
                self.is_main = True
...
class ExerciseComment(models.Model):
    ...
    exercise = models.ForeignKey(
        Exercise, verbose_name=_('Exercise'), editable=False)
