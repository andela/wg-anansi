...
from wger.manager.views import (pdf, schedule, schedule_step, ical, workout,
                                log, set, day, workout_session)

# sub patterns for workout logs
patterns_log = [
    url(r'^(?P<pk>\d+)/view$', log.WorkoutLogDetailView.as_view(), name='log'),
    url(
        r'^(?P<pk>\d+)/edit$',  # JS
...
patterns_workout = [
    ...
    url(r'^(?P<pk>\d+)/delete/$',
        ...
        name='delete'),
    url(r'^(?P<pk>\d+)/view/$', workout.view, name='view'),
    url(r'^calendar/(?P<username>[\w.@+-]+)$', log.calendar, name='calendar'),
    url(r'^calendar$', log.calendar, name='calendar'),
    ...
    url(r'^(?P<pk>\d+)/ical/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$',
        ical.export,
        name='ical'),
    url(r'^(?P<pk>\d+)/ical$', ical.export, name='ical'),
    ...
    url(r'^(?P<id>\d+)/pdf/log/(?P<images>[01]+)/(?P<comments>[01]+)$',
        pdf.workout_log,
        name='pdf-log'),
    url(r'^(?P<id>\d+)/pdf/log$', pdf.workout_log, name='pdf-log'),
...
patterns_day = [
    ...
    url(r'(?P<workout_pk>\d+)/day/add/$',
        ...
        name='add'),
    url(r'(?P<pk>\d+)/delete/$', day.delete, name='delete'),
    url(r'^(?P<id>\d+)/view/$', day.view, name='view'),
    url(r'^(?P<pk>\d+)/log/add/$', log.add, name='log'),
]
...
patterns_set = [
    ...
    url(r'^(?P<pk>\d+)/delete$', set.delete, name='delete'),
    url(r'^(?P<pk>\d+)/edit/$', set.edit, name='edit'),
]

# sub patterns for schedules
patterns_schedule = [
    url(r'^overview$', schedule.overview, name='overview'),
    url(r'^add$', schedule.ScheduleCreateView.as_view(), name='add'),
    url(r'^(?P<pk>\d+)/view/$', schedule.view, name='view'),
    url(r'^(?P<pk>\d+)/start$', schedule.start, name='start'),
    url(r'^(?P<pk>\d+)/edit/$',
    ...
    url(r'^(?P<pk>\d+)/ical/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$',
        ...
        name='ical'),
    url(r'^(?P<pk>\d+)/ical$', ical.export_schedule, name='ical'),
    url(r'^(?P<pk>\d+)/pdf/log/(?P<images>[01]+)/(?P<comments>[01]+)/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$',
    ...
    url(r'^(?P<pk>\d+)/pdf/log/(?P<images>[01]+)/(?P<comments>[01]+)$',
        schedule.export_pdf_log,
        name='pdf-log'),
    url(r'^(?P<pk>\d+)/pdf/log$', schedule.export_pdf_log, name='pdf-log'),
    url(r'^(?P<pk>\d+)/pdf/table/(?P<images>[01]+)/(?P<comments>[01]+)/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$',
